...
//--- mutex --------------------------------------------------------------------
		...
		: pthread_mutex()
	{
		...
		if ( status )
			throw error::runtime( "mutex: " ) << "Unable to create mutex: " <<
				error::number( status );
	}
	...
	mutex::~mutex( void )
	{
		...
		if ( status )
			OOE_WARNING( "mutex", "Unable to destroy mutex: " << error::number( status ) );
	}
...
//--- condition ----------------------------------------------------------------
		...
		: pthread_cond()
	{
		...
		if ( status )
			throw error::runtime( "condition: " ) << "Unable to create condition: " <<
				error::number( status );
	}
	...
	condition::~condition( void )
	{
		...
		if ( status )
			OOE_WARNING( "condition", "Unable to destroy condition: " << error::number( status ) );
	}
	...
	void condition::wait( lock& lock )
	{
		...
		if ( status )
			throw error::runtime( "condition: " ) << "Unable to wait on condition: " <<
				error::number( status );
	}
	...
	void condition::notify_one( void )
	{
		...
		if ( status )
			throw error::runtime( "condition: " ) << "Unable to signal condition: " <<
				error::number( status );
	}
	...
	void condition::notify_all( void )
	{
		...
		if ( status )
			throw error::runtime( "condition: " ) << "Unable to broadcast condition: " <<
				error::number( status );
	}
...
//--- read_write ---------------------------------------------------------------
		...
		: pthread_rwlock()
	{
		...
		if ( status )
			throw error::runtime( "read_write: " ) << "Unable to create read write lock: " <<
				error::number( status );
	}
...
//--- lock ---------------------------------------------------------------------
	...
	lock::~lock( void )
	{
		...
		if ( status )
			OOE_WARNING( "lock", "Unable to unlock mutex: " << error::number( status ) );
	}
...
//--- read_lock ----------------------------------------------------------------
	...
	read_lock::~read_lock( void )
	{
		...
		if ( status )
			OOE_WARNING( "read_lock", "Unable to unlock read: " << error::number( status ) );
	}
...
//--- write_lock ---------------------------------------------------------------
	...
	write_lock::~write_lock( void )
	{
		...
		if ( status )
			OOE_WARNING( "write_lock", "Unable to unlock write: " << error::number( status ) );
	}
}
