...
namespace ooe
{
	...
	inline std::string stack_trace( void ) OOE_INLINE;

//--- error::runtime -----------------------------------------------------------
	class OOE_VISIBLE error::runtime
...
//--- error::time --------------------------------------------------------------
	...
	private:
		template< typename type >
			friend type& operator <<( type& out, const time& )
		{
			std::time_t now = std::time( 0 );
			c8 in[ 9 ];
			strftime( in, sizeof( in ), "%H:%M:%S", localtime( &now ) );
			return out << in;
		}
	};
