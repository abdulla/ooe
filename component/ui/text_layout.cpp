...
struct marker
{
    ...
    marker( text_iterator i_, string_iterator j_, checked_map* map_, f32 width_ )
    ...
    state& state, const marker& next, marker& line, marker& word, const text_iterator& i_end )
{
    ...
    for ( string_iterator j = next.j, j_end = text.data.end(); j != j_end; )
    {
        ...
        {
            word = marker( next.i, j, next.map, 0 );
            continue;
        }
        ...
        if ( state.x + advance > width && prior != line.j )
        {
            ...
            state.last_index = 0;
            j = prior;
            line = word = marker( next.i, j, next.map, 0 );
            continue;
        }

        texture.load( glyph._0.width, glyph._0.height, glyph._1, glyph._2, level );
        add_glyph( glyph, text.colour, state, *next.map, limit.size, slide, level );
